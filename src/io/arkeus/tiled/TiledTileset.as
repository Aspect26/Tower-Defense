package io.arkeus.tiled {
	...
	public class TiledTileset {
		...
		/** Path to the source of the tileset (if present, the rest of values are probably not specified). */
		...
		/** An array of tiles in the order as they were defined. */
		public var tilesOrdered: Array;
		 ...
		 */
		private function loadTiles(tmx:XMLList):void {
			tiles = {};
			tilesOrdered = [];
			...
			for (var i:uint = 0; i < tmx.length(); i++) {
				...
				tiles[node.id] = node;
				tilesOrdered[tilesOrdered.length] = node;
			}
		}
	}
}
